...
class ReplayBuffer:
    ...
    def insert(self, global_obs, local_obs, action, reward, done, available_actions, v_value, next_obs=None):
            ...
            if not self.episode_dones[n]:
                for i in range(n_agents):
                    ...
                    if next_obs is not None:
                        step.append(next_obs[n][i].tolist())
        ...
        for agent_trajectory in episode:
            ...
            for i in reversed(range(len(agent_trajectory))):
                if len(agent_trajectory[i]) == 6:  
                    ...
                    agent_trajectory[i] = agent_trajectory[i][:7]
                else:
                    raise NotImplementedError(f"Unexpected step length: {len(agent_trajectory[i])}")
