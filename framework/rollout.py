...
class RolloutWorker:
    ...
    def __init__(self, model, critic_model,buffer, global_obs_dim, local_obs_dim, action_dim,trainer=None):
        self.buffer = buffer
        self.model = model
    ...
    def rollout(self, env, ret, train=True, random_rate=0.):
        ...
        while True:
            ...
            if train:
                ...
                self.buffer.insert(cur_global_obs, cur_local_obs, action, rewards, 
                                 dones, cur_ava, v_value, next_obs=obs)
