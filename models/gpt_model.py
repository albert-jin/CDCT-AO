...
class Causal_Convolution(nn.Module):
    def __init__(self, config):
        super().__init__()
        assert config.n_embd % config.n_head == 0
        self.window_size = getattr(config, 'window_size', 3)
        self.model_type = config.model_type
        self.remove_act_embs = (self.model_type == 'state_only')
    ...
    def forward(self, x, layer_past=None):
        ...
        conv_tensor = conv_tensor[:, :T, :]
        conv_tensor = self.proj(conv_tensor)
        conv_tensor = self.resid_drop(conv_tensor)
        return conv_tensor
